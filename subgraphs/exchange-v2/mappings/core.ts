...
import { updatePairDayData, updatePairHourData, updateDonaswapDayData, updateTokenDayData } from "./dayUpdates";
import {
  findEthPerToken,
  getETHPriceInUSD,
...
export function handleSync(event: Sync): void {
  ...
  donaswap.totalLiquidityETH = donaswap.totalLiquidityETH.minus(pair.trackedReserveETH as BigDecimal);
  ...
  donaswap.totalLiquidityETH = donaswap.totalLiquidityETH.plus(trackedLiquidityETH);
  donaswap.totalLiquidityUSD = donaswap.totalLiquidityETH.times(bundle.ethPrice);
...
export function handleMint(event: Mint): void {
  ...
  donaswap.totalTransactions = donaswap.totalTransactions.plus(ONE_BI);
...
export function handleSwap(event: Swap): void {
  ...
  let donaswap = DonaswapFactory.load(FACTORY_ADDRESS);
  donaswap.totalVolumeUSD = donaswap.totalVolumeUSD.plus(trackedAmountUSD);
  donaswap.totalVolumeETH = donaswap.totalVolumeETH.plus(trackedAmountETH);
  donaswap.untrackedVolumeUSD = donaswap.untrackedVolumeUSD.plus(derivedAmountUSD);
  donaswap.totalTransactions = donaswap.totalTransactions.plus(ONE_BI);
  ...
  let donaswapDayData = updateDonaswapDayData(event);
  let token0DayData = updateTokenDayData(token0 as Token, event);
  let token1DayData = updateTokenDayData(token1 as Token, event);

  // swap specific updating
  donaswapDayData.dailyVolumeUSD = donaswapDayData.dailyVolumeUSD.plus(trackedAmountUSD);
  donaswapDayData.dailyVolumeETH = donaswapDayData.dailyVolumeETH.plus(trackedAmountETH);
  donaswapDayData.dailyVolumeUntracked = donaswapDayData.dailyVolumeUntracked.plus(derivedAmountUSD);
